        ...
        @foreach ($dataProgramStudis as $index => $prodi)
            ...
            <div class="prodi-item-wrapper">
                <button class="prodi-header-btn manual-toggle-btn {{ $index == 0 ? 'active' : '' }}" 
                        type="button" 
                        data-target="#customCollapse{{ $prodi->id }}">
                ...
                {{-- KONTEN (MANUAL ID) --}}
                <div id="customCollapse{{ $prodi->id }}" 
                     class="prodi-content-area {{ $index == 0 ? 'show-content' : '' }}">
                    
                    @if($prodi->dosens->count() > 0)
                        <div class="row g-4">
                                ...
                                <div class="col-lg-6">
                                    <div class="dosen-card-figma">
                                        <div class="dosen-foto-wrapper">
                                            @if($dosen->foto)
                                                <img src="{{ asset('storage/' . $dosen->foto) }}" class="dosen-foto" alt="Foto">
                                            @else
                                        ...
                                        <div class="dosen-content">
                                            <div class="dosen-nama">
                                                <a href="{{ route('dosen.detail', $dosen->id) }}" class="text-white text-decoration-none">
                                                    {{ $dosen->nama_lengkap }}
                                                </a>
                                            </div>
                                            <div class="info-box">
                                                <div class="data-row">
                                                    ...
                                                    <div class="pill-value">{{ $dosen->nip ? $dosen->nip : '-' }}</div>
                                                </div>
                                                <div class="data-row">
                                                    <div class="pill-label bg-red">JABATAN</div>
                                                    <div class="pill-value">
                                                        {{ $dosen->jabatan_fungsional ? Str::limit($dosen->jabatan_fungsional, 20) : '-' }}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
